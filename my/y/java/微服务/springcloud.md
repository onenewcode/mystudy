...
修改order-service服务中的cn.onenewcode.order.service包下的OrderService类中的queryOrderById方法：
...
修改order-service服务中的cn.onenewcode.order.service包下的OrderService类中的queryOrderById方法。修改访问的url路径，用服务名代替ip、端口：
...
```xml
<dependency>
    <groupId>cn.onenewcode.demo</groupId>
...
这是因为UserClient现在在cn.onenewcode.feign.clients包下，

而order-service的@EnableFeignClients注解是在cn.onenewcode.order包下，不在同一个包，无法扫描到UserClient。
...
```java
@EnableFeignClients(basePackages = "cn.onenewcode.feign.clients")
```
...
> **需求**：给所有进入userservice的请求添加一个请求头：Truth=onenewcode is freaking awesome!
  ...
  cloud:
    gateway:
      ...
      - id: user-service 
        ...
        - Path=/user/** 
        filters: # 过滤器
        - AddRequestHeader=Truth, onenewcode is freaking awesome! # 添加请求头
```
...
spring:
  cloud:
    gateway:
      ...
      - AddRequestHeader=Truth, onenewcode is freaking awesome! 
```
